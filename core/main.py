...
if __name__ == "__main__":
    ...
    precission, recall, mapMeasures, time = MeasureVectorialModel(documents, consults, vectorizer)
    ...
    vectorizer = Vectorizer([str(i) for i in documents], True)
    precission, recall, mapMeasures, time = MeasureProbabilisticModel(documents, consults, vectorizer)
    print(mean(precission), mean(recall), mean(mapMeasures), mean(time))
